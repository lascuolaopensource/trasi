...
# --- `indirizzo`: la distanza dal punto geocodificato ---------------------------------------------------------
#
# Deterministici: Nominatim e Overpass sono mockati e il database è un doppio che risponde alle cinque query di
# `vicino_a` (Casa, memoria, parametri, fonte OSM, fonti `api`). Il bar in memoria ha coordinate **uguali** al punto
# che Nominatim restituisce: se la distanza fosse ancora misurata dalla Casa varrebbe ~2 km, dal centro vale 0.

ENDPOINT_NOMINATIM = "https://nominatim.openstreetmap.org/search"
NOME_FONTE_NOMINATIM = "OpenStreetMap contributors (ODbL) — Nominatim"

CASA_SANBAO = {"id": 5, "slug": "san-bao", "nome": "San Bao", "raggio_m": 800, "lat": 40.60609, "lon": 17.95196}

# Il punto che Nominatim restituisce per «via Appia 120»: circa 2 km a nord-ovest della Casa.
CENTRO_LAT, CENTRO_LON = 40.6225, 17.9400

BAR_KB = {
    "id": 7,
    "nome": "Bar della memoria",
    "tipo": "bar",
    "indirizzo": "Via Appia 122",
    "orari": None,
    "orari_testo": None,
    "lat": CENTRO_LAT,
    "lon": CENTRO_LON,
    # La distanza **dalla Casa** calcolata dal database: se `indirizzo` funziona non deve comparire in risposta.
    "distanza_m": 2099.0,
    "url": None,
    "data_aggiornamento": None,
    "affidabilita": 3,
    "fonte_nome": "Rete-kb-3",
    "fonte_autorita": "Rete delle Case di Quartiere",
}

FONTE_OSM_RIGA = {"nome": "OpenStreetMap-2", "autorita": NOME_FONTE_OSM, "livello_fiducia": 2, "url": None}
FONTE_NOMINATIM_RIGA = {
    "fonte": NOME_FONTE_NOMINATIM,
    "tecnica": "Nominatim-2",
...
class SessioneGeo:
    """Sessione senza database per `vicino_a`: risponde per forma della query, con dati scelti dal test."""

    def __init__(self, *, fonti_api: list[dict] | None = None, fiducia_min: int = 2) -> None:
        self.fonti_api = [FONTE_PUGLIA_RIGA, FONTE_NOMINATIM_RIGA] if fonti_api is None else fonti_api
        self.parametri_valori = {"raggio_vicinanza_m": 800, "fiducia_min_esterna": fiducia_min, "max_risultati_esterni": 5}
    ...
    async def fetch(self, sql: str, *args):
        ...
        if "tipo_accesso = 'api'" in sql:
            return self.fonti_api
        return []

    async def fetchrow(self, sql: str, *args):
        if "osm_overpass" in sql:
            return FONTE_OSM_RIGA
...
@pytest.fixture
def sessione_geo():
    """Installa `SessioneGeo` come dipendenza di sessione dell'app reale, e la rimuove alla fine."""
    from app import db
    from app.main import app

    def installa(**kwargs) -> SessioneGeo:
        doppio = SessioneGeo(**kwargs)
...
def _risposta_nominatim(*risultati: dict) -> httpx.Response:
    """La risposta di Nominatim `format=jsonv2`: una lista (vuota = indirizzo non trovato), coordinate come stringhe."""
    return httpx.Response(200, json=list(risultati))


VIA_APPIA = {"lat": str(CENTRO_LAT), "lon": str(CENTRO_LON), "display_name": "Via Appia, 120, Brindisi, Puglia, Italia"}


@respx.mock
def test_vicino_a_indirizzo_geocodificato_misura_dal_centro(client, sessione_geo):
    """Con `indirizzo` il centro è il punto di Nominatim: `centro` compilato, distanze KB **e** Overpass misurate da lì.

    Il bar in memoria sta esattamente sul punto geocodificato: la sua `distanza_m` deve essere 0, non i 2.099 m dalla
    Casa che il database aveva calcolato. La query Overpass deve usare le coordinate del centro, e `fonti_esterne`
    dichiara entrambe le fonti nell'ordine in cui sono state consultate.
    """
    sessione_geo()
    nominatim = respx.get(ENDPOINT_NOMINATIM).mock(return_value=_risposta_nominatim(VIA_APPIA))
    poi = dict(BAR_APERTO, lat=CENTRO_LAT + 0.001, lon=CENTRO_LON)  # ~111 m a nord del centro
    overpass = respx.post(ENDPOINT_OVERPASS).mock(return_value=_risposta_overpass([poi]))

    risposta = _vicino_a(client, "casa=san-bao&tipo=bar&indirizzo=via+Appia+120")

    assert risposta.status_code == 200, risposta.text
    corpo = risposta.json()
    assert corpo["casa"] == "san-bao", "la Casa resta il contesto"
    assert corpo["centro"] == {
        "lat": CENTRO_LAT,
        "lon": CENTRO_LON,
        "etichetta": VIA_APPIA["display_name"],
        "fonte": NOME_FONTE_NOMINATIM,
    }

    per_nome = {item["nome"]: item for item in corpo["items"]}
    assert per_nome[BAR_KB["nome"]]["distanza_m"] == 0.0, "la distanza KB va ricalcolata dal centro, non dalla Casa"
    assert 100 < per_nome[BAR_APERTO["tags"]["name"]]["distanza_m"] < 125

    assert [(voce["fonte"], voce["stato"]) for voce in corpo["fonti_esterne"]] == [
        (NOME_FONTE_NOMINATIM, "ok"),
        (NOME_FONTE_OSM, "ok"),
    ]

    richiesta = nominatim.calls.last.request
    assert richiesta.headers["User-Agent"].startswith("Trasi/")
    assert b"bounded=1" in richiesta.url.query and b"countrycodes=it" in richiesta.url.query
    assert f"around:800,{CENTRO_LAT},{CENTRO_LON}" in overpass.calls.last.request.content.decode()


@respx.mock
def test_vicino_a_senza_indirizzo_centro_nullo_e_distanza_dalla_casa(client, sessione_geo):
    """Senza `indirizzo` nulla cambia: `centro: null`, la distanza KB è quella del database, una sola fonte esterna."""
    sessione_geo()
    nominatim = respx.get(ENDPOINT_NOMINATIM).mock(return_value=_risposta_nominatim(VIA_APPIA))
    respx.post(ENDPOINT_OVERPASS).mock(return_value=_risposta_overpass([]))

    corpo = _vicino_a(client, "casa=san-bao&tipo=bar").json()

    assert corpo["centro"] is None
    assert not nominatim.called
    assert corpo["items"][0]["distanza_m"] == 2099.0
    assert [voce["fonte"] for voce in corpo["fonti_esterne"]] == [NOME_FONTE_OSM]


@respx.mock
def test_vicino_a_indirizzo_non_trovato_422(client, sessione_geo):
    """Nominatim risponde ma non trova nulla: è un parametro sbagliato (422 con l'indirizzo), non un guasto della fonte."""
    sessione_geo()
    respx.get(ENDPOINT_NOMINATIM).mock(return_value=_risposta_nominatim())
    overpass = respx.post(ENDPOINT_OVERPASS).mock(return_value=_risposta_overpass([]))

    risposta = _vicino_a(client, "casa=san-bao&tipo=bar&indirizzo=via+che+non+esiste+999")

    assert risposta.status_code == 422
    dettaglio = risposta.json()["detail"]
    assert dettaglio.startswith("indirizzo non trovato")
    assert "via che non esiste 999" in dettaglio
    assert not overpass.called


@respx.mock
def test_vicino_a_nominatim_timeout_200_dichiarato(client, sessione_geo):
    """Nominatim in timeout: 200, `items: []`, `centro: null`, `fonti_esterne` con il solo Nominatim; Overpass non si chiama.

    Senza centro nessuna distanza sarebbe vera — né quella dalla Casa, che l'operatore ha detto di non volere.
    """
    sessione_geo()
    respx.get(ENDPOINT_NOMINATIM).mock(side_effect=httpx.ConnectTimeout("timeout simulato"))
    overpass = respx.post(ENDPOINT_OVERPASS).mock(return_value=_risposta_overpass([BAR_APERTO]))

    risposta = _vicino_a(client, "casa=san-bao&tipo=bar&indirizzo=via+Appia+120")

    assert risposta.status_code == 200, risposta.text
    corpo = risposta.json()
    assert corpo["items"] == [] and corpo["centro"] is None
    assert corpo["casa"] == "san-bao" and corpo["tipo"] == "bar" and corpo["raggio_m"] == 800
    assert [(voce["fonte"], voce["stato"]) for voce in corpo["fonti_esterne"]] == [(NOME_FONTE_NOMINATIM, "timeout")]
    assert corpo["fonti_esterne"][0]["ms"] >= 0
    assert not overpass.called


@respx.mock
def test_vicino_a_nominatim_errore_http_200_dichiarato(client, sessione_geo):
    """Un 403 di Nominatim (UA rifiutato) è `stato: errore`, non un 5xx dello shim."""
    sessione_geo()
    respx.get(ENDPOINT_NOMINATIM).mock(return_value=httpx.Response(403, text="Access blocked"))

    corpo = _vicino_a(client, "casa=san-bao&tipo=bar&indirizzo=via+Appia+120").json()

    assert corpo["items"] == [] and corpo["fonti_esterne"][0]["stato"] == "errore"


@pytest.mark.parametrize(
    "kwargs",
    [
        pytest.param({"fonti_api": [FONTE_PUGLIA_RIGA]}, id="nominatim_assente_dalla_allowlist"),
        pytest.param({"fiducia_min": 3}, id="nominatim_sotto_soglia"),
    ],
)
@respx.mock
def test_vicino_a_indirizzo_senza_fonte_nominatim_ammessa_422(client, sessione_geo, kwargs):
    """Senza una riga Nominatim attiva e sopra soglia, `indirizzo` non è servibile: 422 esplicito, nessuna chiamata."""
    sessione_geo(**kwargs)
    nominatim = respx.get(ENDPOINT_NOMINATIM).mock(return_value=_risposta_nominatim(VIA_APPIA))
    ...
    assert risposta.status_code == 422
    assert "geocodifica non disponibile" in risposta.json()["detail"]
    assert not nominatim.called


def test_vicino_a_indirizzo_troppo_corto_422(client, sessione_geo):
    """Il contratto dichiara `minLength: 3`: due caratteri non sono un indirizzo e non arrivano a Nominatim."""
    sessione_geo()

    assert _vicino_a(client, "casa=san-bao&tipo=bar&indirizzo=vi").status_code == 422
