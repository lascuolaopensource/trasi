...
async def eventi_mese(
    ...
    **Fuori dal contratto congelato** (`include_in_schema=False`): non è uno strumento del LLM, è la lettura del
    ...
    righe = await sess.fetch(SQL_EVENTI, slug, dal, al, None)
    ...
    # Le stesse occorrenze e lo stesso ordine di `eventi_oggi`: un evento ricorrente compare in ogni giornata del
    # mese in cui cade, e la lista è per data dell'occorrenza — la Home mostra il calendario, non la riga d'origine.
    eventi: list[ItemEvento] = []
    for riga in righe:
        for occ_inizio, occ_fine in _occorrenze(riga, dal, al):
            eventi.append(_item_evento(riga, occ_inizio, occ_fine))
    eventi.sort(key=lambda e: (e.data, e.ora_inizio or ""))

    return RispostaEventiMese(casa=slug, dal=dal, al=al, oggi=oggi, eventi=eventi)


async def _slug_casa_richiesta(casa: str | None, sess: Sessione) -> str:
    """Lo slug della Casa di una lettura del calendario **di una Casa sola**: il parametro, o la Casa dell'operatore.

    Il modello scrive il nome («San Bao») più spesso dello slug: si prova a riconoscerlo. Solo se non è una Casa
    riconoscibile si ripiega su quella dell'operatore (v. `routes_geo`). La usa `eventi_mese`, che è per Casa;
    `eventi_oggi` no, perché senza `casa` legge tutta la rete.
    """
